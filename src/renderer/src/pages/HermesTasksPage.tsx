...
function loadTasks(): HermesTask[] {
  try {
    const raw = localStorage.getItem(STORAGE_KEY)
    if (!raw) return []
    const parsed = JSON.parse(raw) as HermesTask[]
...
function saveTasks(list: HermesTask[]) {
  try {
...
export function HermesTasksPage() {
  const setModule = useAppStore((s) => s.setModule)
  const [tasks, setTasks] = useState<HermesTask[]>(loadTasks)
  ...
  return (
    <main className="workspace">
      <div className="page-header">
        ...
        <div>
          <h1>Hermes 定时任务</h1>
          <div className="sub">让 Hermes 按固定周期自动执行提示词，结果汇总到产物库</div>
        </div>
      ...
      <div className="card" style={{ flex: 1, minHeight: 0, display: 'flex', flexDirection: 'column' }}>
        {sorted.length === 0 ? (
          <div className="empty-state" style={{ flex: 1 }}>
            ...
            <div className="empty-state-sub">创建周期性提示词任务，Hermes 会自动运行并把结果保存到产物库。</div>
            <button className="btn btn-primary btn-sm" onClick={() => setAdding(true)}>
              <Plus size={13} style={{ marginRight: 4 }} />
              新建任务
            </button>
